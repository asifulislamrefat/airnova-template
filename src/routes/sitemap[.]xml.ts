import { createFileRoute } from "@tanstack/react-router";
import type {} from "@tanstack/react-start";
import { PROJECTS } from "@/lib/projects";

const BASE_URL = "https://airnova-template.lovable.app";

interface SitemapEntry {
  path: string;
  changefreq?: "always" | "hourly" | "daily" | "weekly" | "monthly" | "yearly" | "never";
  priority?: string;
}

const BLOG_SLUGS = [
  "how-to-create-content-that-actually-converts",
  "where-creativity-meets-strategy",
  "explore-the-future-of-digital-design",
  "innovative-thinking-for-digital-success",
  "creative-tips-for-modern-designers",
  "your-source-for-design-inspiration",
];

const SERVICE_SLUGS = [
  "brand-identity-design",
  "creative-consulting",
  "prototype-wireframing",
  "e-commerce-design",
  "dashboard-saas-design",
  "graphic-design",
];

export const Route = createFileRoute("/sitemap.xml")({
  server: {
    handlers: {
      GET: async () => {
        const entries: SitemapEntry[] = [
          { path: "/", changefreq: "weekly", priority: "1.0" },
          { path: "/about", changefreq: "monthly", priority: "0.8" },
          { path: "/studio", changefreq: "monthly", priority: "0.7" },
          { path: "/services", changefreq: "monthly", priority: "0.8" },
          { path: "/projects", changefreq: "weekly", priority: "0.8" },
          { path: "/pricing", changefreq: "monthly", priority: "0.7" },
          { path: "/blog", changefreq: "weekly", priority: "0.8" },
          { path: "/contact", changefreq: "monthly", priority: "0.7" },
          ...SERVICE_SLUGS.map((slug) => ({ path: `/services/${slug}`, changefreq: "monthly" as const, priority: "0.6" })),
          ...PROJECTS.map((p) => ({ path: `/projects/${p.slug}`, changefreq: "monthly" as const, priority: "0.6" })),
          ...BLOG_SLUGS.map((slug) => ({ path: `/blog/${slug}`, changefreq: "monthly" as const, priority: "0.6" })),
        ];

        const urls = entries.map((e) =>
          [
            `  <url>`,
            `    <loc>${BASE_URL}${e.path}</loc>`,
            e.changefreq ? `    <changefreq>${e.changefreq}</changefreq>` : null,
            e.priority ? `    <priority>${e.priority}</priority>` : null,
            `  </url>`,
          ]
            .filter(Boolean)
            .join("\n"),
        );

        const xml = [
          `<?xml version="1.0" encoding="UTF-8"?>`,
          `<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">`,
          ...urls,
          `</urlset>`,
        ].join("\n");

        return new Response(xml, {
          headers: {
            "Content-Type": "application/xml",
            "Cache-Control": "public, max-age=3600",
          },
        });
      },
    },
  },
});